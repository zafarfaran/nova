...
export default async function OnboardingPage({ params }: PageProps) {
    ...
    const vatPeriodLabel = latestVatPeriod
        ? `Q${Math.ceil((latestVatPeriod.periodStart.getMonth() + 1) / 3)} ${latestVatPeriod.periodStart.getFullYear()}`
        : "No period";
            ...
            <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
                ...
                <div className="mb-6 overflow-hidden rounded-2xl bg-white shadow-lg ring-1 ring-slate-200 sm:mb-8">
                    ...
                    <div className="grid gap-4 p-6 sm:grid-cols-2 sm:gap-6 sm:p-8 lg:grid-cols-3">
                        <InfoCard label="Email" value={client.contactEmail || "Not provided"} icon="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                        <InfoCard label="Entity Type" value={client.entityType} icon="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                        <InfoCard label="VAT Scheme" value={client.vatScheme || "Not set"} icon="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" />

                        {latestVatPeriod && (
                            <div className="sm:col-span-2 lg:col-span-3">
                                <InfoCard
                                    label="VAT Period"
                                    value={vatPeriodLabel}
                                    subtitle={`${new Date(latestVatPeriod.periodStart).toLocaleDateString()} - ${new Date(latestVatPeriod.periodEnd).toLocaleDateString()}`}
                                    icon="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"
                                />
                            </div>
                        )}

                        {client.salesChannels.length > 0 && (
                            <div className="sm:col-span-2 lg:col-span-3">
                                <ListCard label="Sales Channels" items={client.salesChannels} icon="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                            </div>
                        )}

                        {client.notes && (
                            <div className="sm:col-span-2 lg:col-span-3">
                                <InfoCard label="Notes" value={client.notes} icon="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
                            </div>
                        )}
                    </div>
                </div>
                        ...
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                                ...
                                <ul className="mt-3 space-y-2">
                                    {client.autoChasers.map((chaser: { delayDays: number }, idx: number) => (
                ...
                {/* Submit Button */}
                <SubmitButton clientId={client.id} checklistItems={client.checklistItems} />
            </div>
        </div>
    );
}
