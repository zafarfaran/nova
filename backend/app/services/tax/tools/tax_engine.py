...
def execute_compute_tax_position(
    ...
    try:
        ...
        if pension_contrib > 0 or employer_contrib > 0 or gift_aid_val > 0:
            logger.warning(
                "BRB-extending inputs passed to engine, pension_contributions=%s, "
                "employer_contributions=%s, gift_aid=%s",
                pension_contrib,
                employer_contrib,
                gift_aid_val,
            )
        ...
        logger.info(
            "Tax engine computed, total_income=%s, total_tax=%s",
...
def execute_model_salary_sacrifice(
    tool_input: dict, *, context: dict | None = None
) -> dict:
    ...
    try:
        ...
        logger.info(
            "Salary sacrifice modelled, total_saving=%s",
...
def execute_model_personal_pension(
    tool_input: dict, *, context: dict | None = None
) -> dict:
    ...
    try:
        ...
        logger.info(
            "Personal pension modelled, total_saving=%s",
            result["savings"]["total"],
        )
