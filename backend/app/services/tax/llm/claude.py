...
from app.services.tax.tools import execute_tool
...
OBSERVATION_TOOLS = [
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "potential_saving": {
                    "type": "number",
                    "description": "Estimated annual tax saving in pounds (optional, only if quantifiable)",
                },
            },
...
class ClaudeProvider:
    def __init__(self):
        ...
        logger.info("Claude provider initialized, model=%s", self.model)
    ...
    def stream_chat(
        ...
        logger.info(
            "Starting Claude stream, message_count=%d, model=%s",
            len(messages),
            self.model,
        )
        ...
        try:
            ...
            for _round in range(max_rounds):
                ...
                with self.client.messages.stream(**{**api_kwargs, "messages": messages}) as stream:
                    for event in stream:
                        if event.type == "content_block_start":
                            ...
                            if current_tool_id and current_tool_name:
                                ...
                                try:
                                    ...
                                    logger.error(
                                        "Failed to parse tool input JSON, tool=%s, raw=%s",
                                        current_tool_name,
                                        raw_json[:500],
                                    )
                                    tool_input = {}
                                ...
                                logger.info(
                                    "Tool call detected, tool=%s, tool_id=%s, tool_input=%s",
                                    current_tool_name,
                                    current_tool_id,
                                    tool_input,
                                )
                ...
                if tool_called:
                    ...
                    yield StatusEvent(phase=StatusPhase.UNDERSTANDING)
                    logger.info(
                        "Continuing after tool call, round=%d",
                        _round + 1,
                    )
                else:
                ...
                if tool_called:
                    logger.warning(
                        "Max tool rounds exhausted — final response may be incomplete, max_rounds=%d",
                        max_rounds,
                    )

            logger.info(
                "Claude stream completed, input_tokens=%d, output_tokens=%d",
                input_tokens,
                output_tokens,
            )
            ...
            logger.error(
                "Claude API error, error=%s, status_code=%s",
                str(e),
