...
Adapted from Helio's context router (helio/apps/api/app/routers/context.py).
Changes from Helio original:
  ...
  - ContextSnippet -> TaxContextSnippet from app.models.tax
  - _clean_with_llm kept as async (Anthropic client is async) — called via
    asyncio.run() for sync compatibility
...
from app.models.tax import TaxContextSnippet

logger = logging.getLogger(__name__)

router = APIRouter(tags=["tax-context"])
...
def ingest_context(
    ...
    logger.info(
        "Context ingest received, snippet_id=%s, source_url=%s, capture_type=%s, raw_length=%d",
        snippet_id,
        body.source_url,
        body.capture_type,
        len(raw),
    ...
    logger.info("Pre-cleaned content, raw_length=%d, cleaned_length=%d", len(raw), len(pre_cleaned))

    # Step 2: LLM cleanup for semantic structuring
    cleaned = _clean_with_llm_sync(pre_cleaned)

    snippet = TaxContextSnippet(
        id=snippet_id,
        user_id=user_id,
    ...
    logger.info("Context snippet stored, snippet_id=%s, cleaned_length=%d", snippet_id, len(cleaned))
...
def get_pending_context(
    ...
    result = session.execute(
        ...
        .where(TaxContextSnippet.user_id == user_id)
        .where(TaxContextSnippet.is_consumed == False)  # noqa: E712
        .where(TaxContextSnippet.status == "ready")
        .order_by(TaxContextSnippet.created_at.desc())
    )
    snippets = list(result.scalars().all())

    logger.debug("Pending context fetched, count=%d", len(snippets))
...
def dismiss_context(
    ...
    user_id = "demo-user"
    result = session.execute(
        update(TaxContextSnippet)
        .where(TaxContextSnippet.id == snippet_id)
        .where(TaxContextSnippet.user_id == user_id)
    ...
    logger.info("Context snippet dismissed, snippet_id=%s", snippet_id)
    return {"ok": True}
...
def _clean_with_llm_sync(raw_content: str) -> str:
    """Synchronous wrapper around the async LLM cleanup."""
    ...
    try:
        ...
        logger.exception("LLM cleanup failed, returning pre-cleaned content, error=%s", str(e))
